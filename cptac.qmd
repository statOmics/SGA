...
Unlike for real experiments, we know the ground truth for this dataset: UPS
...
data preprocessing functionality. A major advantage of QFeatures is that it provides
...
We remove peptides that could not be mapped to a protein. We also remove peptides that cannot be uniquely mapped to a single protein because their sequence is shared across multiple proteins. This often occurs for homologs or for proteins that share similar functional domains. Shared peptides are often mapped to protein groups, which are the set of proteins that share the given peptides. Protein groups are encoded by appending the constituting protein identifiers, separated by a `";"`.
...
found across all samples (`nNA` is 0) and others that could not be
...
log2 intensity, this basically boils down to calculating log-ratios between
...
The method first calculates a pseudo-reference sample (row-wise mean of log2
intensities, which corresponds to the log2 transformed geometric mean), then
calculates log2 ratios of each sample w.r.t. the pseudo-reference, and finally
takes the column wise median of these log2 ratios to obtain the sample-based
normalisation factor on the log2 scale. This procedure is implemented in the
`msqrob2` function `nfLogMedianOfRatios`.
...
with $y_{ip}$ the log2-transformed intensities and $nf_i$ the log2-transformed
...
Note that this allows for a flexible parameterisation of the treatment
...
`proteins` assay in `qf` under the column names of the contrast matrix `L`.

Users can directly access the results tables via the colData. However, `msqrob2` 
