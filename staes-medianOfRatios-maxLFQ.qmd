...
We load the output from a DIA-NN parquet file. This can be a file path to a local file or a url to a file that lives on the web.
...
are observed in at least 66.7% of the samples, which roughly corresponds to 2 samples per condition. This is an arbitrary
...
default function `FUN = "-"`, `MARGIN = 2` to subtract the column wise log2
